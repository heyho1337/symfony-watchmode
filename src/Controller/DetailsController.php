...
class DetailsController extends AbstractController
{
    ...
    public function sendDetailData(Request $request, string $id): Response
    {
		try {
            ...
            return $this->json(['message' => 'Form submission failed'], Response::HTTP_BAD_REQUEST);
        } catch (\Exception $e) {
            // Log the exception
            $this->logger->error('An error occurred: ' . $e->getMessage());

            // Return a JSON response with an error message
            return $this->json(['message' => 'An error occurred while processing the form'], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
}
